...
/**
 * Sistema de Proteção Contra Duplicatas para MongoDB Atlas - Versão Corrigida
 */
...
class ProtecaoMongoDBCorrigida {
  constructor() {
    this.client = null;
    this.db = null;
  ...
  async salvarAnuncioComProtecao(dadosAnuncio) {
    try {
      ...
      const dadosCompletos = {
        ...
        updatedAt: new Date(),
        versao: 1,
        firebaseId: null // Evitar conflito com índice existente
      };
...
async function executarTeste() {
  const protecao = new ProtecaoMongoDBCorrigida();
